...
public abstract class TeleportService extends WearableListenerService{
    ...
    public abstract void onMessageReceived(String path,byte[] data);
    ...
    public void onMessageReceived(final MessageEvent messageEvent) {
        Log.d(TAG, "onMessageReceived() A message from watch was received:" + messageEvent.getRequestId() + " " + messageEvent.getPath());
            ...
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    TeleportService.this.onMessageReceived(messageEvent.getPath(),
        ...
        if(!flagHandled && onGetMessageCallback != null) {
            ...
            onGetMessageCallback.onCallback(messagePath,messagePath.getBytes());
            flagHandled = true;
        }

        if (!flagHandled && onGetMessageTask != null) {
            String messagePath = messageEvent.getPath();
            onGetMessageTask.execute(messagePath,messagePath.getBytes());
            flagHandled = true;
        }
